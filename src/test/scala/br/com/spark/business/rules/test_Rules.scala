...
class test_Rules extends FunSuite with SharedSparkContext with DatasetSuiteBase with Checkers {
  val views = ConfigUtil.code_view.keys.toSeq.sortWith(_.toInt < _.toInt)
  val grp_metrics = ConfigUtil.code_grp_metric.keys.toSeq.sortWith(_.toInt < _.toInt)
  for (view <- views) {
    for (grp_metric <- grp_metrics) {
