...
class QtyFormatsUDAF extends UserDefinedAggregateFunction with SalesMetricTrait {
  ...
  override def update(buffer: MutableAggregationBuffer, input: Row): Unit = {
    if(buffer != null) {
      ...
      buffer(10) =  if (metric("store_other")){ buffer(10).asInstanceOf[Double] + 1 } else { buffer(10).asInstanceOf[Double] }
