...
class QtyServUDAF extends UserDefinedAggregateFunction with SalesMetricTrait {
  ...
  override def update(buffer: MutableAggregationBuffer, input: Row): Unit = {
    if(buffer != null) {
      ...
      buffer(5) =  if (metric("hyp"))         { buffer(5).asInstanceOf[Double]  } else { buffer(5).asInstanceOf[Double] }
      buffer(6) =  if (metric("sup"))         { buffer(6).asInstanceOf[Double]  } else { buffer(6).asInstanceOf[Double] }
      buffer(7) =  if (metric("prx"))         { buffer(7).asInstanceOf[Double]  } else { buffer(7).asInstanceOf[Double] }
      buffer(8) =  if (metric("drg"))         { buffer(8).asInstanceOf[Double]  } else { buffer(8).asInstanceOf[Double] }
      buffer(9) =  if (metric("gas"))         { buffer(9).asInstanceOf[Double]  } else { buffer(9).asInstanceOf[Double] }
      ...
      buffer(12) =  if (metric("app"))        { buffer(12).asInstanceOf[Double] + 1 } else { buffer(12).asInstanceOf[Double] }
      buffer(13) =  if (metric("ecm_std"))    { buffer(13).asInstanceOf[Double] + 1 } else { buffer(13).asInstanceOf[Double] }
      buffer(14) =  if (metric("click"))      { buffer(14).asInstanceOf[Double] + 1 } else { buffer(14).asInstanceOf[Double] }
      buffer(15) =  if (metric("drive"))      { buffer(15).asInstanceOf[Double] + 1 } else { buffer(15).asInstanceOf[Double] }
      buffer(16) =  if (metric("scan"))       { buffer(16).asInstanceOf[Double] + 1 } else { buffer(16).asInstanceOf[Double] }
      buffer(17) =  if (metric("rappi"))      { buffer(17).asInstanceOf[Double] + 1 } else { buffer(17).asInstanceOf[Double] }
      buffer(18) =  if (metric("ecm_other"))  { buffer(18).asInstanceOf[Double] + 1 } else { buffer(18).asInstanceOf[Double] }
