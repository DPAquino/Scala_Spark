...
class ValAvgPriceUDAF extends UserDefinedAggregateFunction with SalesMetricTrait {
  ...
  override def update(buffer: MutableAggregationBuffer, input: Row): Unit = {
    if (buffer != null) {
      ...
      val lst_cod_bus_items: Map[String, Double] = 
          lst_sales_items.
          filter(i => !i.isNullAt(idx_cod_bus)).
          map(i => (
            i.get(idx_cod_bus).toString
            , i.get(idx_qty_item).asInstanceOf[java.math.BigDecimal].doubleValue() 
          )).
          groupBy(_._1).mapValues(s => s.map(_._2).reduce( _ + _ ))
      val lst_cod_bus_gasto: Map[String, Double] =
          lst_sales_items.
          filter(i => !i.isNullAt(idx_cod_bus)).
          map(i => (
            i.get(idx_cod_bus).toString
      ...
      val cod_bus = lst_cod_bus_gasto.keys.toSeq
      ...
      val metric_buffer_qty = metric_classifier.buffer(metric, lst_cod_bus_items.values.sum, lst_cod_bus_items.getOrElse("FOOD", 0.0), lst_cod_bus_items.getOrElse("NFOOD", 0.0))
      val metric_buffer_val = metric_classifier.buffer(metric, lst_cod_bus_gasto.values.sum, lst_cod_bus_gasto.getOrElse("FOOD", 0.0), lst_cod_bus_gasto.getOrElse("NFOOD", 0.0))
      //---------Items------------------
