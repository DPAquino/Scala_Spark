...
class GrupoTransform extends Transform {
  ...
  def transform(dataframe:DataFrame): DataFrame = {  
    ...
    withColumn(
      ...
      SalesModel.ItemField.DS_GROUP
      , col(s"${SalesModel.Field.LST_SALES_ITEMS}.${SalesModel.ItemField.DS_GROUP}")
    ).
    groupBy(
      col(SalesModel.ItemField.DS_GROUP)
